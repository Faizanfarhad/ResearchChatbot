...
Usage:
    ...
    python evaluation/evaluation.py --doc docsContainer/1810.04805v2.pdf --top_k 10 --generate_answer --llm_device cpu
"""
...
def hallucination_check(answer: str, sources: List[str]) -> float:
    ...
    Checks max Jaccard overlap between answer tokens and any source chunk.
    """
    if not sources:
        return 1.0
...
def run_evaluation(
    ...
    print(f"LLM answer generation: {'ON' if generate_answer else 'OFF'} "
          f"(model={llm_model_name}, device={llm_device})")

    # Initialize pipeline with LLM config
    ...
    for i, q in enumerate(questions):
        ...
        # Time the query (with optional LLM generation)
        t_start = time.time()
        try:
            response = pipeline.query(question, generate_answer=generate_answer)
        except Exception as e:
            print(f"  ERROR: {e}")
            continue
        t_end = time.time()
    ...
    print(f"Latency:   avg={result.retrieval.avg_latency_ms:.0f}ms")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--doc", type=str, required=True,
                        help="Path to document (PDF/HTML/MD)")
    parser.add_argument("--questions", type=str,
                        default="evaluation/bert_test_questions.json")
    ...
    parser.add_argument("--generate_answer", action="store_true",
                        help="Use local LLM to generate answers (requires generator.py)")
    parser.add_argument("--llm_model", type=str,
                        default="Qwen/Qwen2.5-1.5B-Instruct",
                        help="HF model name for answer generation")
    parser.add_argument("--llm_device", type=str, default="cpu",
                        help="Device for LLM inference (cpu or cuda)")
